...
class Bid:
    ...
    def __init__(self, number, price, guarantee_price, commission, days_count, type, publish_date, subject, region,
...
class BankBidBasic:
    ...
    def __init__(self, bid: Bid, principal: Principal, client: Client,
        ...
        self.fields.append(BankField("Итоговая сумма контракта", bid.price.replace('.', ','), True))
...
class BankBidInfoAccordion:
    ...
    def __init__(self, bid: Bid, principal: Principal, factor1="Нет", factor2="Нет", factor3="Нет",
                 factor4="Нет",
                 factor5="Нет"
                 ):
        self.fields = []
        ...
        self.fields.append(BankField("Начальная цена контракта", bid.start_price.replace('.', ','), True))
        self.fields.append(BankField("Предложенная нами цена", bid.price.replace('.', ','), True))
