...
from Pages.BasePage import BasePage, Loaders, Base
from Pages.LoginPage import LoginPage
from classes.entities import Bid, BankAccount, BankAccountOtp, Beneficiary, Client, Stockholder, CommissionExpo
from selenium.webdriver.common.keys import Keys


class AgentPage(BasePage):
    create_bid = '[href="/new/create"]'
    bid_number_filter = '[placeholder="№ заявки"]'

    def check_open(self):
        self.wait_until_located(self.create_bid, 'css')
    ...
    def click_on_create_bid_btn(self):
        sleep(0.5)
        self.click_on_elem(self.create_bid)
...
class CreateBidPage(BasePage):
    ...
    def check_open(self):
        self.wait_until_located(self.inn, 'xpath')
    ...
    def fill_required_fields_client(self, bid: Bid, client: Client, group=False):
        ...
        Loaders(self.browser).wait_until_agent_loader_disappear()
        self.fill_field_and_check_value_click_on_focus(self.contract_price, bid.price, self.page_title)
...
class InfoAboutClientForm(BasePage):
    bank_account_number = '[placeholder="Расчетный счет клиента"]'
...
class CommonPage(BasePage):
    ...
    def fill_required_fields(self, entity, form):
        ...
        self.click_on_elem(form + self.birth_place)  # снять фокус
...
class DocsPage(BasePage):
    ...
    docs_download_btns = ".b-doc__action-list .b-doc__download"

    def get_docs_count(self):
        return len(self.find_elements_by_locator(self.docs_download_btns))

    def submit(self):
        self.wait_until_clickable(self.submit_btn, 'xpath')
        self.click_on_elem(self.submit_btn)
        Loaders(self.browser).wait_until_agent_loader_disappear(50)
    ...
    def load_passport(self, filename, btn_locator=passport, wait_elem=None):
        file_path = Base().get_file_dir('files', filename)
        # if wait_elem is not None:
        #     self.wait_until_located(wait_elem, 'css')
